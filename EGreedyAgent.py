# -*- coding: utf-8 -*-
"""
Created on Mon Dec 10 12:00:38 2018
...
###############################################################################
class Egreedy_agent(bandit_agent):
    ...
    def do_action(self):
        playing_strategy=np.random.binomial(n=1,p=self.epsilon)
        if playing_strategy:
            action=int(np.random.choice(self.actions,size=1))
        else:
...
class env_EGreedyAgent(environment):
